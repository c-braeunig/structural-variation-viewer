...
if __name__=='__main__':
   ...
   parser.add_argument('-C','--contigname',type=str,nargs=1,help='Provide contig/chromosome criterion by which to filter SV records; multiple criteria given space-separated')
   parser.add_argument('-T','--type',type=str,nargs='+',help="Provide type criterion by which to filter SV records i.e. 'DEL'; multiple criteria given space-separated; 'BND' for complex rearrangments with breakends")
   parser.add_argument('-S','--conttype',type=str,nargs='+',help="Verbose type filter: types entered will be omitted. Same usage as --tfil")
