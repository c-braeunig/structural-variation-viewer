...
if __name__ == "__main__":
   ...
   parser=argparse.ArgumentParser(description='Parse and derive SV information from .vcf output of variant calling software; --flag generates a superficial overview of the listed SV types and their number of occurrences. Using --sort, --cfil, --tfil, --vfil, --llim and --ulim, the SVs of intereste can be isolated and printed to stdout or to a file ( --out) in a bed-like file format. For further information on the vcf format consult https://samtools.github.io/hts-specs/VCFv4.2.pdf')
   parser.add_argument('--vcf',help='Provide .vcf file of SVs')
   ...
   parser.add_argument('--flag',action='store_true',help='Choose to display an overview of SVs in the provided vcf file')
   parser.add_argument('--cfil',type=str,nargs=1,help='Provide contig/chromosome criterion by which to filter SV records; multiple criteria given space-separated; use only one for visualization')
   ...
   parser.add_argument('--vfil',type=str,nargs='+',help="Verbose type filter, same usage as tfil")
   ...
   parser.add_argument('--seq',action='store_true',help='Select to write sequence data into fasta file; requires --out')
   parser.add_argument('--nosize',action='store_true',help='Select to not display the end position and size [bp] of each SV in line')
   parser.add_argument('--circos',action='store_true',help='Circos mode; generates karyotype and link files for circos for BND-type SVs to show inter-chromosomal events; compatible w/ cfil')
   parser.add_argument('--out',type=str,help='Provide desired name of bed-formatted output file')
   args=parser.parse_args()

   if args.flag:
      if args.cfil:
         variants=_readfile(args.vcf, cfil=args.cfil, tfil=args.tfil, vfil=args.vfil)
         _getstats(variants)
      else:
         _contig_overview(args.vcf)
   elif args.circos:
      variants=_readfile(args.vcf, cfil=args.cfil, tfil='BND')
