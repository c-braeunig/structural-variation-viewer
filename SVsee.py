...
def _heatmap(features,clen,step,Plot,**kwargs):
    ...
    ax_heat.imshow(y[np.newaxis,:], cmap="plasma", aspect="auto", extent=extent)
    ax_heat.set_yticks([])
...
if __name__ == "__main__":
   ...
   parser=argparse.ArgumentParser(description='Single-chromsome visualization of SV distribution')
   parser.add_argument('--inn',help='Provide input file (SVread output file)')
   ...
   if args.mod and args.heat:
      ...
      _vdraw(args.inn,cname,plot,reg=args.reg)
      _annodraw(args.mod,cname,plot,reg=args.reg)
   elif args.heat:
